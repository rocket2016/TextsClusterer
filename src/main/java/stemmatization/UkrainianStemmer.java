...
public class UkrainianStemmer implements Stemmer {
    ...
    public String stem(String word) {
        ...
        do {
            Matcher m0 = INFINITIVE.matcher(word);
            if (!m0.replaceFirst("").equals(word)) {
                stem = word;
                continue;
            }
            Matcher m = RVRE.matcher(stem);  //RVRE ...
            if (m.matches()) {

                String pre = m.group(1);// the parentheses are not in the group we need
...
//        Stemmer stemmer = new UkrainianStemmer();
//        stemmer.stem("повільне");
//    }
}
